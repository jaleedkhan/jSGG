...
lol = 0 # Global Variable for lengths
BOX_SCALE = 1024  # Scale at which we have the boxes

class VGDataset(torch.utils.data.Dataset):

    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                filter_empty_rels=True, num_im=-1, num_val_im=5000, #num_val_im=5000,
        ...
        if self.custom_eval:
            ...
            self.filenames = [self.filenames[i] for i in range(lol)]
            self.img_info = [self.img_info[i] for i in range(lol)]
