...
def make_data_loader_2(cfg, img, mode='train', is_distributed=False, start_iter=0):
    ...
    lol = os.path.join(cfg.PATHS_CATALOG, "_2.py")

    paths_catalog = import_file(
        "maskrcnn_benchmark.config.paths_catalog_2", lol, True
    )
    DatasetCatalog = paths_catalog.DatasetCatalog(img)
    if mode == 'train':
...
class one_image(torch.utils.data.Dataset):

    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, img, transforms=None,
                filter_empty_rels=True, num_im=-1, num_val_im=1, #num_val_im=5000,
                filter_duplicate_rels=True, filter_non_overlap=True, flip_aug=False, custom_eval=True, custom_path=''):
        """
